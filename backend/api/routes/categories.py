...
from backend.models.database import Category, ProductCategory, Product
...
async def get_all_categories(db: Session = Depends(get_db)):
    """获取所有品类（树形结构）"""
    all_categories = db.query(Category).order_by(Category.sort_order, Category.id).all()
...
async def get_level_one_categories(db: Session = Depends(get_db)):
    """获取所有一级目录"""
...
async def get_level_two_categories(parent_id: int, db: Session = Depends(get_db)):
    """获取指定一级目录下的二级目录"""
    categories = db.query(Category).filter(
...
async def get_product_categories(product_id: int, db: Session = Depends(get_db)):
    """获取产品关联的所有品类"""
    product = db.query(Product).filter(Product.id == product_id).first()
    if not product:
        raise HTTPException(status_code=404, detail="产品不存在")

    associations = db.query(ProductCategory).filter(
...
async def set_product_categories(product_id: int, request: ProductCategoryRequest, db: Session = Depends(get_db)):
    """设置产品关联的品类"""
    product = db.query(Product).filter(Product.id == product_id).first()
    if not product:
        raise HTTPException(status_code=404, detail="产品不存在")
    ...
    if request.category_ids:
        existing_cats = db.query(Category.id).filter(
